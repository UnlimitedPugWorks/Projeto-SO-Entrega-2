...
long maze_fileread (FILE* Istream, FILE* Ostream, maze_t* mazePtr){
    ...
    while (fgets(line, sizeof(line), Istream)) {
        ...
        switch (code) {
            ...
            PARSE_ERROR:
            default: { /* error */
                fprintf(Ostream, "Error: line %li invalid\n",
                        lineNumber);
                exit(1);
            }
        }
     ...
     */
    if (width < 1 || height < 1 || depth < 1) {
        fprintf(Ostream, "Error: Invalid dimensions (%li, %li, %li)\n",
                width, height, depth);
        exit(1);
    }
    grid_t* gridPtr = grid_alloc(width, height, depth);
    ...
    addToGrid(gridPtr, srcVectorPtr,  "source");
    addToGrid(gridPtr, dstVectorPtr,  "destination");
    fprintf(Ostream, "Maze dimensions = %li x %li x %li\n", width, height, depth);
    fprintf(Ostream, "Paths to route  = %li\n", list_getSize(workListPtr));
...
bool_t maze_checkPaths (maze_t* mazePtr, list_t* pathVectorListPtr, FILE* outputfile){
    ...
    while (list_iter_hasNext(&it, pathVectorListPtr)) {
        ...
        for (i = 0; i < numPath; i++) {
            ...
            grid_getPointIndices(gridPtr,prevGridPointPtr,&prevCoordinate.x,&prevCoordinate.y,&prevCoordinate.z);
            ...
            for (j = 1; j < (numPoint-1); j++) { /* no need to check endpoints */
                ...
                grid_getPointIndices(gridPtr,currGridPointPtr,&currCoordinate.x,&currCoordinate.y,&currCoordinate.z);
    ...
    /*if (doPrintPaths) {*/
    fprintf(outputfile,"\n%s\n","Routed Maze:");
    grid_fileprint(outputfile, testGridPtr);
   /* }*/
    grid_free(testGridPtr);
    return TRUE;
}
